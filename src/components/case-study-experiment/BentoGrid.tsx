...
export function ImageTile({
 ...
 sizes={fullWidth ? "(max-width: 768px) 100vw, (max-width: 1440px) 100vw, 1440px" : "(max-width: 768px) 100vw, (max-width: 1440px) 50vw, 720px"}
 className={`w-full h-auto object-contain transition-transform duration-700 ease-out group-hover:scale-[1.02] ${isSketchbook ? 'print-rotate' : ''}`}
 />
 {/* View Focus hover overlay */}
 <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none">
 <span className="bg-black/60 text-white text-[10px] font-bold uppercase tracking-widest px-4 py-2 rounded-full border border-white/10">
 View Focus
 </span>
 </div>
 {caption && (
...
export function VideoTile({
 ...
 <div className="w-20 h-20 rounded-full bg-[var(--bg-ink-900)] flex items-center justify-center border border-white/20 shadow-2xl transition-transform duration-300 hover:scale-105 hover:bg-[var(--bg-ink-800)]">
...
export function CarouselTile({
 ...
 {/* Images */}
 <div
 className="relative cursor-pointer print:flex print:flex-col print:gap-8"
 onClick={() => lightbox?.open(images, current)}
 >
 {images.map((img, idx) => {
 const isSketchbook = img.src.includes('sketchbook');
 return (
 <div key={idx} className={`${idx === current ? 'block' : 'hidden'} print:block`}>
 <Image
 src={img.src}
 alt={img.alt}
 width={1920}
 ...
 className={`w-full h-auto object-contain transition-opacity duration-500 ${isSketchbook ? 'print-rotate' : ''}`}
 />
 {img.caption && (
 <p className="mt-2 text-sm text-center text-zinc-400 print:text-black">
 {img.caption}
 </p>
 )}
 </div>
 ...
 className="absolute left-2 top-1/2 -translate-y-1/2 w-7 h-7 rounded-full bg-black/60 flex items-center justify-center text-zinc-400 hover:text-white hover:bg-black transition-all opacity-0 group-hover:opacity-100"
 aria-label="Previous"
 >
 ...
 className="absolute right-2 top-1/2 -translate-y-1/2 w-7 h-7 rounded-full bg-black/60 flex items-center justify-center text-zinc-400 hover:text-white hover:bg-black transition-all opacity-0 group-hover:opacity-100"
 aria-label="Next"
 >
