...
import PortfolioQuiz from '@/components/quiz/PortfolioQuiz'
...
export default function PortfolioVeil() {
  ...
  const handleEnter = useCallback(() => {
    ...
    window.scrollTo(0, 0)
    // Trigger the theatrical exit
    setIsExiting(true)
    // Delay the actual hide to let the zoom animation play
    setTimeout(() => {
      setShowVeil(false)
    }, 200) // Small delay so zoom animation can start
  }, [])
      ...
      <AnimatePresence mode="wait" onExitComplete={handleAnimationComplete}>
        {showVeil && (
          <motion.div
            ...
            transition={{
              duration: isExiting ? 1.0 : 0.3,
              ease: [0.76, 0, 0.24, 1] // Dramatic ease-out
            }}
            style={{
              pointerEvents: showVeil && !isExiting ? 'auto' : 'none',
          ...
          >
            ...
            >
              ...
              >
                ...
                {/* CTA - Two buttons: Quiz and Enter */}
                <motion.div
                  initial={{ opacity: 0, y: 10 }}
                  animate={{ opacity: 1, y: 0 }}
                  transition={{ delay: 0.7, duration: 0.6 }}
                  className="flex flex-col sm:flex-row items-center justify-center gap-space-3"
                >
                  <button
                    onClick={() => setIsQuizOpen(true)}
                    disabled={!showVeil}
                    className={`group inline-flex items-center justify-center gap-space-2 px-space-5 md:px-space-6 py-space-3 md:py-space-3.5 rounded-full bg-white/10 ${t.text} text-sm md:text-base font-medium hover:bg-white/20 active:scale-[0.98] transition-all hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed border ${t.borderSubtle}`}
                  >
                    <span>🎯</span>
